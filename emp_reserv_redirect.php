...
if($p_date_time == ""||$r_date_time == ""){
  header('Location: emphome.php?error=1');
  die();
}
...
if($result = mysqli_query($conn, $sql)){
  if(mysqli_num_rows($result)==0){
    header('Location: emphome.php?error=2');
    die();
    }
  }
...
serial_no in (select car_serial_no from reservation_log
 ...
 and serial_no not in( select car_serial_no from service_request))
 UNION
 ...
 if($result = mysqli_query($conn, $sql)){
   if(mysqli_num_rows($result)>0){
     $output="";
        ...
        header('Location: emp_availiability.php?value='.$output);
        die();
     }
     else{
        header('Location: emphome.php?error=2');
